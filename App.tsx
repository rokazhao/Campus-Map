...
export class App extends Component<AppProps, AppState> {
      ...
      return <div className="content main">
        <div>
          ...
          <FriendsEditor
            ...
            onAdd={this.doAddFriendClick}
            onRemove={this.doRemoveFriendClick}
          />
        </div>
  ...
  doGetDataJson = (data: unknown): void => {
    ...
    this.setState({ show: "edit", schedule, savedSchedule: schedule, friends: Array.isArray(data.friends) ? data.friends : [] });
  }
  ...
  doAddFriendClick = (friend: string): void => {
    ...
    const newFriends = this.state.friends.concat([friend]);
    this.doSaveFriendsClick(newFriends);
  };

  doRemoveFriendClick = (friend: string): void => {
    if (!this.state.user || !this.state.friends) {
      return;
    }

    const newFriends: string[] = [];
    for (const currentFriend of this.state.friends) {
  ...
  doSaveFriendsClick = (friends: string[]): void => {
    fetch("/api/setData", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user: this.state.user, friends })
    })
      .then(res => res.json())
      .then(() => this.setState({ friends }));
  };
}